...
RTEBase.prototype.endpointParamSpec = function (path)
	{
		...
		specs['tenant/add_new_user_card']   = this.objectBuilder({})
						     .build('user_id', 'required:true, encrypt:false') 
							 .build('card_type', 'required:true, encrypt:false') 
						     .build('number', 'required:true, encrypt:false')
						     .build('issuer', 'required:false, encrypt:false')
						     .build('cvv2', 'required:true, encrypt:false')
						     .build('expiry_month', 'required:true, encrypt:false')
						     .build('expiry_month', 'required:true, encrypt:false')
						     .end();
		
		specs['tenant/add_new_bank_account']   = this.objectBuilder({})
						     .build('user_id', 'required:true, encrypt:false') 
							 .build('name', 'required:true, encrypt:false') 
						     .build('number', 'required:true, encrypt:false')
						     .build('bank_name', 'required:true, encrypt:false')
						     .build('bank_code', 'required:true, encrypt:false')
						     .end();
		
		specs['tenant/delete_user_card']   = this.objectBuilder({})
						     .build('user_id', 'required:true, encrypt:false') 
							 .build('card_number', 'required:true, encrypt:false')
						     .end();
							 
		specs['tenant/delete_user_bank_account']   = this.objectBuilder({})
						     .build('user_id', 'required:true, encrypt:false') 
							 .build('account_number', 'required:true, encrypt:false')
						     .end();
